...
final class NetworkingClientTestCase: XCTestCase {
    ...
    private func verifyRequest(
        ...
        let networkingClient = NetworkingClientMock(networkRequestHandler: NetworkRequestHandlerMock(urlSession: .shared))
...
private class NetworkingClientMock: NetworkingClient {
    ...
    var networkRequestHandler: NetworkRequestHandler

    var line: UInt?
    var onPerformRequest: ((_ request: URLRequest, _ line: UInt) -> Void)?

    required init(networkRequestHandler: any StytchCore.NetworkRequestHandler) {
