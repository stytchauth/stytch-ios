...
final class AsyncMethodsTestCase: BaseTestCase {
    ...
    func testPasswords() async throws {
        // MARK: - Creation
        var data = try Current.jsonEncoder.encode(DataContainer(data: StytchClient.Passwords.CreateResponse(requestId: "321", statusCode: 200, wrapped: .init(emailId: "email_id_that's_what_i_am"))))
        var request: URLRequest?
        Current.networkingClient = .mock(verifyingRequest: { request = $0 }, returning: .success(data))
        let passwordParams: StytchClient.Passwords.PasswordParameters = .init(email: "user@stytch.com", password: "password123", sessionDuration: 26)
        ...
        // MARK: Authenticate
        Current.timer = { _, _, _ in .init() }
        data = try Current.jsonEncoder.encode(DataContainer(data: AuthenticateResponse.mock))
        Current.networkingClient = .mock(verifyingRequest: { request = $0 }, returning: .success(data))
        ...
        // MARK: - Strength check
        data = try Current.jsonEncoder.encode(DataContainer(data: StytchClient.Passwords.StrengthCheckResponse(requestId: "123", statusCode: 200, wrapped: .init(validPassword: false, score: 20, breachedPassword: true, feedback: .init(suggestions: [], warning: "meh. do something.")))))
        Current.networkingClient = .mock(verifyingRequest: { request = $0 }, returning: .success(data))
        ...
        // MARK: - Reset start
        data = try Current.jsonEncoder.encode(DataContainer(data: BasicResponse(requestId: "123", statusCode: 200)))
        Current.networkingClient = .mock(verifyingRequest: { request = $0 }, returning: .success(data))
