...
public extension StytchB2BClient {
    ...
    static func handle(url: URL, sessionDurationMinutes: Minutes, completion: @escaping Completion<DeeplinkHandledStatus<DeeplinkResponse<B2BMFAAuthenticateResponseDataType>, DeeplinkTokenType, DeeplinkRedirectType>>) {
        Task {
            do {
    ...
    ///    - url: A `URL` passed to your application as a deeplink.
    ///    - sessionDurationMinutes: The duration, in minutes, of the requested session. Defaults to 5 minutes.
    static func handle(url: URL, sessionDurationMinutes: Minutes) -> AnyPublisher<DeeplinkHandledStatus<DeeplinkResponse<B2BMFAAuthenticateResponseDataType>, DeeplinkTokenType, DeeplinkRedirectType>, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
