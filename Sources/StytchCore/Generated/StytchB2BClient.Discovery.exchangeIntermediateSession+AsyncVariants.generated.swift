...
public extension StytchB2BClient.Discovery {
    ...
    func exchangeIntermediateSession(parameters: ExchangeIntermediateSessionParameters, completion: @escaping Completion<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>>) {
        Task {
            do {
    ...
    /// Wraps Stytch's [exchange intermediate session](https://stytch.com/docs/b2b/api/exchange-intermediate-session) endpoint. This operation consumes the `intermediate_session_token`. If this method succeeds, the Member will be logged in, and granted an active session.
    func exchangeIntermediateSession(parameters: ExchangeIntermediateSessionParameters) -> AnyPublisher<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
