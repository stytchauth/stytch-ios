...
public extension StytchB2BClient.Discovery {
    ...
    func createOrganization(parameters: CreateOrganizationParameters, completion: @escaping Completion<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>>) {
        Task {
            do {
    ...
    /// Wraps Stytch's [create Organization via discovery](https://stytch.com/docs/b2b/api/create-organization-via-discovery) endpoint. This operation consumes the `intermediate_session_token`. If this method succeeds, the Member will be logged in, and granted an active session.
    func createOrganization(parameters: CreateOrganizationParameters) -> AnyPublisher<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
