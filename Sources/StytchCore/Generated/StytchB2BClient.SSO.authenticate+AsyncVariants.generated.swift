...
public extension StytchB2BClient.SSO {
    ...
    func authenticate(parameters: AuthenticateParameters, completion: @escaping Completion<SSOAuthenticateResponse>) {
        Task {
            do {
    ...
    /// Authenticate a member given a token. This endpoint verifies that the memeber completed the SSO Authentication flow by
    /// verifying that the token is valid and hasn't expired.
    func authenticate(parameters: AuthenticateParameters) -> AnyPublisher<SSOAuthenticateResponse, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
