...
public extension StytchClient.Passwords {
    ...
    func resetBySession(parameters: ResetBySessionParameters, completion: @escaping Completion<PasswordsSessionResetResponse>) {
        Task {
            do {
    ...
    /// 
    /// The provided password needs to meet our password strength requirements, which can be checked in advance with the password strength endpoint. If the token and password are accepted, the password is securely stored for future authentication and the user is authenticated.
    func resetBySession(parameters: ResetBySessionParameters) -> AnyPublisher<PasswordsSessionResetResponse, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
