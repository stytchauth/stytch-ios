...
public extension StytchB2BClient.Sessions {
    ...
    func exchange(parameters: ExchangeParameters, completion: @escaping Completion<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>>) {
        Task {
            do {
    ...
    /// Use this endpoint to exchange a Member's existing session for another session in a different Organization.
    func exchange(parameters: ExchangeParameters) -> AnyPublisher<B2BMFAAuthenticateResponse<B2BMFAAuthenticateResponseData>, Error> {
        return Deferred {
            Future({ promise in
                Task {
                    do {
