...
public struct NetworkingRouter<Route: RouteType> {
    private let getConfiguration: () -> StytchClientOptions?
    ...
    private init(_ pathForRoute: @escaping (Route) -> Path, getConfiguration: @escaping () -> StytchClientOptions?) {
...
extension NetworkingRouter where Route: BaseRouteType {
    init(getConfiguration: @escaping () -> StytchClientOptions?) {
