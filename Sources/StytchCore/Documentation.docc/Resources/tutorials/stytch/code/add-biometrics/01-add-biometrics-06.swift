...
final class AuthenticationViewController: UIViewController {
    ...
    private func promptForBiometricRegistration(user: User) {
        ...
        alertController.addAction(.init(title: "OK", style: .default) { [weak self] _ in
            self?.registerBiometrics(identifier: user.emails.first?.email ?? user.id.rawValue)
        })
        present(alertController, animated: true)
    }

    private func registerBiometrics(identifier: String) {
        Task {
            do {
                let authResponse = try await StytchClient.biometrics.register(parameters: .init(identifier: identifier))
